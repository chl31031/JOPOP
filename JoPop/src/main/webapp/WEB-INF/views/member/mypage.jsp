	          	...
	          	<div class="section review-section">
				    <c:if test="${not empty reviews}">
				        <c:forEach var="review" items="${reviews}">
				            ...
				            <div class="review-divider"></div>    <!-- 구분선 -->
				        </c:forEach>
				    </c:if>
...
<script>
	...
	$(".btn_modify").on("click", function(e){
	    const modifyForm = $(this).parent().find('#modify-form');
    ...
    function submitReviewForm(imageList) {
        ...
        $.ajax({
            type: 'POST',
            url: '${pageContext.request.contextPath}/member/modifyReview',
            data: reviewFormData,
            processData: false,
            contentType: false,
            dataType: 'json',
    ...
    function fileCheck(fileName, fileSize) {
        // 파일의 최대 크기를 나타내는 변수
        var maxSize = 500; // 예시 값, 실제로는 적절한 크기로 설정되어야 합니다.

        // 파일 이름의 유효성을 검사하는 정규식
        var regex = /^[a-zA-Z0-9]+\.(jpg|jpeg|png)$/; // 예시 값, 적절한 정규식으로 변경되어야 합니다.
